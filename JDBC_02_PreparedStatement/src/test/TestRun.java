...
//		selectTest();
	...
	public static void selectTest() {
		...
		//  * 데이터가 있는 지 여부 확인 => rset.next():boolean (데이터가 있으면 true, 없으면 false)
			while(rset.next()) {
				...
				Date tdate = rset.getDate("TDATE");	// java.sql.Date 객체임!
	...
	public static void insertTest() {
		...
		try {
			...
			conn.setAutoCommit(false);	// 자동 커밋 옵션 false로 설정 (jdbc6 버전 이후는 auto commit 설정)
			...
			System.out.println("[ERROR] SQL 오류발생!!" + e.getMessage());
		} finally {
